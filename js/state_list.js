...
function showHideOtherState(stateListBox)
{
	...
	{
		document.getElementById('other_state_text').innerHTML = "<div><label class='frm-label'>Other State:</label><input type='text' name='other_state' class='frm-field' value=''></div>";
	}
	else
	{
...
{
	handle = document.getElementsByName("state")[0];
	...
	else
	{
		...
		for(i=0 ; i < cnt; i++)
		{
			handle.options[i] = new Option(stripsp(stateData[i][0]), stateData[i][1]);
		}
	}
}
...
function change_country(countrycode)
{
	...
	if ( countrycode != "" )
	{
		...
		document.getElementById('other_state_text').innerHTML = "";
		url = "http://www.bigrock.in/misc/getState.php?countrycode="+countrycode;
        if(countryCodeList[countrycode] != undefined){
            document.getElementById('input_phone_cc').value = countryCodeList[countrycode];
            if(typeof document.sign != 'undefined')
            {
                if(sign.mobileno != undefined)
                    document.getElementById('input_mobile_cc').value = countryCodeList[countrycode];
            }
            if(typeof document.signup != 'undefined')
            {
                if(signup.mobileno != undefined)
                    document.getElementById('input_mobile_cc').value = countryCodeList[countrycode];
            }
        }
		loadXMLDoc(url);
